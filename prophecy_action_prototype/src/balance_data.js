...
PA.Balance = (function () {
  ...
  function text(run) { const id = (run && run.balance) || PA.BALANCE_DEFAULT, B = PA.BALANCE_SETS[id] || PA.BALANCE_SETS.current; const diff = PA.DIFFICULTY.candidates[(run && run.difficulty) || B.difficulty || 'base'] || PA.DIFFICULTY.candidates.base; const H = PA.BOSS_HP_SETS[B.bossHpSet]; const dh = (run && run.dayHpSet) || B.dayHp || 'none'; return `v${PA.VERSION} · ${B.name}${dh !== 'none' ? ' · 날짜 체력 ' + dh : ''} · 적 체력 ${Object.values(diff.hp).join('/')} · 보스 체력 ${H.boss.stage1}/${H.guardian.stage2}/${H.eater.stage3} · 처치 경험치 ×${B.killXp} · 지역 경험치 ×${B.bonusXp}${B.spear && B.spear.interval ? ' · 창 주기 ' + B.spear.interval + '·근접 약화' : ''}`; }
