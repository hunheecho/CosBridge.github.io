...
PA.Render = (function () {
  ...
  function drawWeaponBodies(ctx, st) {
    ...
    for (const w of st.weapons || []) {
      if (w.stats.kind === 'orbit' && w.bladePos) for (const bp of w.bladePos) { if (bp.ix != null) { ctx.save(); ctx.strokeStyle = 'rgba(143,182,238,0.35)'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(bp.ix, bp.iy); ctx.lineTo(bp.x, bp.y); ctx.stroke(); ctx.restore(); } ctx.save(); ctx.translate(bp.x, bp.y); ctx.rotate(bp.a + st.t * 8); ctx.fillStyle = '#e6edf5'; ctx.strokeStyle = '#8fb6ee'; ctx.lineWidth = 1.5; for (let i = 0; i < 3; i++) { ctx.rotate(TAU / 3); ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(14, -4); ctx.lineTo(16, 0); ctx.lineTo(14, 4); ctx.closePath(); ctx.fill(); ctx.stroke(); } ctx.restore(); }
