...
| R-V08-15 | 검증 2차: 합의 사항 코드 대조(기본 새 회차 실제 실행), 시작 기술 3종 짝 비교(A/B/C/D, 공통 정책 vs 기술 이해 정책), 보스 맞으면서 공격(행동 4 × 빌드 6~7), 지속 피해·발동 감사, 날짜별 경제, 악용 경로 | 플레이 전 기계적 확인 | 7 | ✅ | `docs/sim/COMPARE_v08.md` 검증 2차 절, `test/abuse_v08.test.js`, `test/dot_audit_v08.test.js`, `tools/verify_menu_v08.js` | 재미·최종 밸런스 승인 아님 |
| R-V08-16 | 검증 메뉴(제목): 시작 기술 첫 전투·같은 투자·관문 빌드 보스전·3일차 중단 최종·상점 비교·통계 화면, 검증용 회차 표시·기록 제외, 시험실 밸런스 세트 선택 | 돌아와서 바로 비교 | 7 | ✅ | `tools/verify_menu_v08.js` 20/20(index·dist) | 검증용 회차는 현재 저장을 덮어씀(경고 표시) |
| R-V08-17 | 회전 칼날 시험값 8/0.45 → 10/0.35(측정 근거 기록) | 시작 기술 3종이 같은 수준에서 출발 | 7 | 🟡 | `docs/ASSUMPTIONS.md` 검증 2차, `PAIRED_E.md` | 확정 아님, 플레이 뒤 판단 |
