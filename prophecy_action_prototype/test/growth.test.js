...
test('경험치는 적 사망 즉시, 처치 원인과 무관, 같은 적 1회, 동시 처치 합산, 바닥 구슬 없음', () => {
  ...
  CB.step(st, {}, PA.CONFIG.STEP);
  assert.equal(g.level, 2); assert.equal(g.xp, 13 + 6 - 14, '파편 처치 + 레벨업');
  assert.equal(st.pickups.length, 0, '경험치 구슬 없음');
  const d = CB.spawnEnemy(st, 'wolf_alpha', 400, 100); d.hp = 1; d.summoned = true; CB.damageEnemy(st, d, 5, {}); assert.equal(g.xp, 5 + 6, '소환 적은 소환 경험치');
});
