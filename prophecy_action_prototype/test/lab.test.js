...
test('빌드 프리셋 8종 이상이 모두 게임 규칙(슬롯·레벨·방식·전제)을 지키고 선택 횟수가 계산된다', () => {
  ...
  for (const id of ids) { const d = L.describeBuild(id); assert.equal(d.errors.length, 0, id + ': ' + d.errors.join(',')); assert.ok(d.picks >= 0); assert.ok(['초반', '중간', '후반', '관문 1', '관문 2', '관문 3', '비교', '시작 비교', '투자 비교'].includes(d.stage)); }
