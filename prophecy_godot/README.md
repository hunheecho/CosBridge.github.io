...
## 조작
WASD/방향키 이동 · Space 회피 · Q 감속장 · 자동 공격(검격 Lv1) · Esc 일시정지/재개 · F3 검증 패널(개발용) · Enter 시작/재시작.
...
- `scripts/game/` 표시·연결부(`combat_view.gd` 고정 단계 누적기와 `_draw`, `main.gd` 화면 전환·HUD, `game.gd` 자동 로드).
