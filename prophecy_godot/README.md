# 텐데이즈투둠스데이 (Ten Days to Doomsday) — Godot 이식 (prophecy_godot)

> 옛 가칭 "예언의 시간표". 2026-09-10 에 이름이 확정됐다. 아래 과거 기록의 옛 제목은 역사 기록이라 그대로 둔다.

HTML 프로토타입(`../prophecy_action_prototype`, v0.8.0, 커밋 ee10fc7)에서 합의된 게임 전체를 Godot으로 옮긴 프로젝트입니다(godot-0.5.0: 전체 이식 0.4.0 + 검수 수정 + 세계 변화 + 반복 콘텐츠 + 영구 성장 시험 프로필 + UI·모바일 준비). 새 회차 → 시작 기술 → 거점 → 장소/시간대 카드 → 전투(레벨업 3택) → 승리/보상 → 더 깊이/귀환 → 상점·장비·대장간·휴식 → 다음 날 → 관문 보스(희귀 보상) → 최종 보스 → 회차 결과 → 새 회차까지 일반 UI로 진행할 수 있고, 저장/계속하기와 패배 경로가 있습니다. 0.3.1의 첫 전투(사용자 긍정 평가, D33)는 "기준 전투"로 그대로 보존됩니다. 게임 이름은 **텐데이즈투둠스데이 / Ten Days to Doomsday** 로 확정됐습니다(2026-09-10).

이식 기준·충돌 처리: `../docs/PORT_BASELINE.md` · 콘텐츠 ID별 상태: `../docs/CONTENT_MATRIX.md` · 규칙 계층 계약: `docs/PORT_CONVENTIONS.md` · 잠정값과 근거: `docs/ASSUMPTIONS.md` · 이번 이식 기록·재현 정보: `docs/PORT_NOTES.md` §12(0.4.0)·§13~§16(0.4.1~0.5.0).
