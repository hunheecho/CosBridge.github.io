...
| 항목 | 결과 |
|---|---|
| 실제 UI 경로 전체 회차 (`ui_smoke_full`, 헤드리스, 시드 1, 실력 봇, 경로 고정) | **`UI_SMOKE result=done` · 종료 코드 0**. 관문 1 = 81초, 관문 2 = 151초, 관문 3 = 294초, **10일차 회차 결과 = 294초**, 저장→종료→계속하기 = 82초. 실제 경과 318초, 게임 속 전투 시간 1548.8초. 회차 결과 뒤 무한 모드·새 회차 화면까지 진행. 넘은 관문 `["boss","guardian","eater"]`, 최종 레벨 22 |
