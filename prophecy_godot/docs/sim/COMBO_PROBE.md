...
생성: `tools/combo_probe.gd` (Windows, Godot 4.7.2-stable (official)). 9일차 수준 편성(늑대 4·궁수 2·방패병 1). 봇 실력 두 가지(skilled·novice). 상한 90초.
**모든 조합의 대규모 시뮬레이션이 아니다.** 사용자가 지정한 대표 조합 7개만 돌린다.
...
| 조합 | 무엇을 보려는가 | 읽는 지표 |
|---|---|---|
| daggers_orb_echo | 감전·연타 연계와 재귀 발동 검사 | shock_procs, copies, copy_dmg |
| hammer_frost_doll | 적 모으기·공격 기회 | slows, taunted, soaked |
| spear_wind_mine | 거리 유지·추격 경로 | push_dist, fires |
| sword_plague_crow | 우선 처치와 전염 | spreads, bursts, marks |
...
**지표가 0일 때 두 경우를 구분한다.** 발동 자체가 0이면 봇이 그 효과를 쓸 줄 모르는 것일 수 있고,
발동은 되는데 결과가 작으면 콘텐츠 수치 문제일 수 있다. 0을 곧바로 '약하다'로 적지 않는다.
