...
>
> 2026-09-09 3차 확정: **부활 체력 25%가 실제 재도전까지 유지된다**(§2 "부활 뒤 관문 재입장").
> 예전에는 관문 입장이 체력을 완전 회복시켜 25%가 관문 사망에서 아무 비용도 아니었다. 이제 부활로 들어온 재입장
> **한 번**만 자동 회복을 건너뛴다. 그 사이 회복 수단을 썼으면 오른 체력 그대로 들어가고(다시 25%로 깎지 않는다),
> 정상적인 관문 입장의 완전 회복 규칙은 그대로다.
...
* `run.revivePending = { count: int }` — **부활로 들어가는 관문 재입장 표식**(1이면 다음 관문 입장 한 번이 자동 완전 회복을 건너뛴다).
  `PRun.revive_pending(run)` / `PRun.clear_revive_pending(run)`. 같은 `{ count: int }` 모양이라 저장 정규화 표를 건드리지 않는다.
...
다른 관문보다 관대하다 — 그 대가가 물약 1개(220금) **+ 25% 체력으로 하는 재도전**이다.

### 부활 뒤 관문 재입장 [확정 — 2026-09-09 3차]

관문 입장(`PRun.start_boss`)은 원래 **체력을 완전 회복**시켰고, 전투를 만드는 `PFlow.make_boss_encounter`도 최대 체력으로 시작했다.
그래서 관문에서 죽고 부활한 뒤 다시 들어가면 25%가 그대로 100%가 되어 **25%가 실제로 무는 것이 없었다**.
사용자가 의도한 것은 **25% 체력으로 재도전하는 것**이므로 다음과 같이 고쳤다.

* **표식**: 부활이 회차를 `phase == "boss_prep"`(관문 앞)에 세우면 `run.revivePending = { count: 1 }`이 붙는다.
  마지막 날 부활(같은 날)도, 다음 날이 관문 날이라 관문 앞으로 부활하는 경우도 같다.
  보통 날 아침(`phase == "prep"`)으로 부활하면 붙이지 않는다 — 그 사이 `end_day`가 정상 회복을 하므로 남길 이유가 없다.
* **소비**: `PRun.start_boss`가 표식을 보면 그 입장에서 **자동 완전 회복을 건너뛰고** 표식을 지운다.
  성공한 관문 입장 **한 번**에만 쓰인다. `end_day`(하루가 정상으로 넘어감)·`boss_victory`(관문 돌파)·`boss_defeat_retry`(시험 재시도 경로)도 표식을 지운다.
* **전투 시작 체력**: `PFlow.make_boss_encounter`가 시작 체력을 직접 정하지 않고 `PRun.boss_start_hp(run, build)`에게 묻는다.
  그 값은 `run.hp`를 최대 체력으로 자른 것이라, **보통 입장은 예전과 똑같이 최대 체력**이고 부활 재입장만 25%다.
  최대 체력(`player.hp_max`)은 깎지 않는다 — 25/100으로 시작할 뿐이다.
* **회복 수단**: 부활 뒤 휴식·회복약·휴식권으로 체력을 올렸으면 **그 체력 그대로** 들어간다(다시 25%로 깎지 않는다).
  마지막 날에는 남은 시간이 0이라 휴식(`PRun.can_rest`는 `phase == "prep"`만 허용)이 막히고, **시간이 들지 않는 회복약**만 쓸 수 있다.
* **시험 재시도 경로와 분리**: `run.testRetry` 회차는 사망 정산 자체를 하지 않으므로 표식이 서지 않는다. 재도전 입장은 예전대로 완전 회복이다.
* **저장 호환**: 옛 저장에는 이 키가 없고, 없으면 표식이 없는 것으로 읽는다(`PRun.revive_pending`). `PSave` 정규화 표는 건드리지 않았다.

`tests/death_tests.gd` §13이 **사망 → 부활 → 저장·복구 → 재입장 → `CombatState.player.hp` → 두 번째 사망**까지 끊지 않고 못박는다.

> 남은 관찰(밸런스 판단은 사람 몫): 이 변경으로 관문 사망의 값이 **물약 1개 + 그날 남은 시간 + 25% 체력 재도전**이 되었다.
...
| `PRun.revive_pending(run)` | `run` | `bool` | **다음 관문 입장에 자동 회복이 없는가**(부활 재입장 표식). 관문 준비 화면·패배 화면의 경고 문구 |
| `PRun.boss_start_hp(run, build := {})` | `run`, 선택 `build` | `float` | 관문 전투가 시작될 때의 체력. 준비 화면의 "입장 체력 n/최대" 표시와 `PFlow.make_boss_encounter`가 함께 쓴다 |
...
| 함수 | 쓰는 곳 |
|---|---|
| `PDefeatScreen.death_lines(run)` · `PDefeatScreen.next_label(run)` · `no_heal_line(run)` | 일반 출격 패배 화면 |
| `PBossResultScreen.defeat_lines(run)` · `retry_label(run)` · `can_retry_now(run)` · `no_heal_line(run)` | 관문 패배 화면 |

`no_heal_line`은 `PRun.revive_pending(run)`이 true인 동안에만 한 줄을 준다("다시 들어가도 체력은 자동으로 차지 않습니다").
표식이 소비되면 빈 문자열이라 정상 입장 안내와 섞이지 않는다. 관문 준비 화면(`base.gd`)도 같은 판정으로 경고 한 줄과
"입장 체력 n/최대"를 보여 준다.
...
* **소유 범위를 한 줄 넘은 곳(사람이 확인할 것)**: 2026-09-09 3차 확정을 넣으면서 `scripts/rules/flow.gd`의
  `make_boss_encounter` 한 줄(`"hp": float(b.hp_max)` → `"hp": PRun.boss_start_hp(run, b)`)을 바꿨다.
  전투 시작 체력을 정하는 자리가 거기뿐이라 규칙 계층만으로는 25%가 전투까지 갈 수 없었다.
  **판정은 전부 `PRun.boss_start_hp`에 있고 flow.gd에는 규칙 숫자를 두지 않았다.** 균형 수치는 건드리지 않았다.
