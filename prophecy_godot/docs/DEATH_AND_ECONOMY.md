...
* `run.death = { key, cause("sortie"|"boss"), day, stage, count, seq, regionId, bossId, revived, endedRun, nextDay, hp }`
...
### 경계 규칙 [시험 규칙 — 사용자 합의 없음]

* 부활해도 **미완료 관문을 건너뛰지 않고 다음 막을 해금하지 않는다.**
  관문일에 죽어 부활하면 다음 날에도 `phase == "boss_prep"`이고 `run.stage`·`run.bossesDone`은 그대로다.
* 날짜를 넘긴 뒤에도 미완료 관문이 우선이다. 그 관문을 넘기 전까지 출격(`PRun.can_sortie`)이 잠긴다.
* 부활 시각의 체력은 `round(최대 체력 × 0.25)`이며 최소 1이다.

### 마지막 날(다음 날이 없는 경우) — **결정 항목**

`PRun.has_next_day(run)`가 false인 날(본편 10일차 = 마지막 관문일)에는 "다음 날"이 없다.
**지금 코드는 아래 (나)를 [시험 규칙]으로 넣어 두었다.** 사용자가 고르면 그 자리만 바꾸면 된다(`PRun.settle_death`의 `next_day` 분기).

| 후보 | 내용 | 근거 |
|---|---|---|
| (가) 쓰지 못하고 회차 종료 | 마지막 날에는 물약이 무용지물 | "남은 하루를 잃고 다음 날"이라는 확정 문구를 글자 그대로 지킨다. 규칙이 가장 단순하고 예외가 없다. 다만 **마지막 관문 직전에 산 물약이 값어치를 잃어** 상점 결정이 날짜에 따라 달라진다(플레이어가 알기 어렵다). |
| **(나) 쓰되 날짜를 넘기지 않고 그 날 남은 시간을 전부 잃는다** ← 현재 구현 | 같은 날 관문 앞에서 체력 25%로 다시 선다 | "남은 하루를 잃는다"를 **"그 날의 남은 시간을 잃는다"**로 읽는다. 물약의 값어치가 회차 어디서나 같아 상점 결정이 흔들리지 않고, 일정(10일)을 늘리지 않는다. 대신 **마지막 관문만 25% 체력 재도전이 가능**해져 다른 관문보다 관대하다. |
| (다) 11일차를 열어 관문만 재도전 | 일정을 하루 늘린다 | 확정 문구("다음 날")를 가장 곧이곧대로 만족시킨다. 대신 **10일 일정 자체가 깨지고** 결과 화면·기록·막 구조가 "10일"이라는 전제를 잃는다. |
...
| `run.death` (필드) | — | `Dictionary` | 사망 화면: `cause`·`day`·`revived`·`count`. `revived`가 true면 부활 안내, false면 회차 종료 안내 |
...
* **화면**: 마을 UI의 판매·휴식 확인 창, 사망 화면 문구(현재 `scripts/game/screens/defeat.gd`는
  "구조되어 … 정상 체력으로 시작합니다"라는 옛 규칙 문구를 그대로 쓴다), `boss_result.gd`의 재도전 버튼(사람 플레이에서는 항상 잠긴다).
