...
# ---------- 4. 독 전염 — 세대 상한 2에서 사슬이 멈추는가 ----------
## data/supports.json은 읽기만 한다. 비교를 위해 **메모리에 올라온 자격표 사전**의 gen_max만 잠시 바꾼다
## (파일은 건드리지 않는다). 끝나면 원래 값으로 되돌린다.
##
## **여기는 경로 확인이지 상한 비교표가 아니다.** 늑대 체력을 1로 두어 감염된 적이 즉사하게 만들었으므로
## 세대 상한이 사슬을 끊는 유일한 것이 된다 — 그래야 "상한이 실제로 작동하는가"만 볼 수 있다.
## 그 대가로 **이 조건의 비율은 실제 전투의 효과 감소율이 아니다.** 체력이 실제 값에 가까워지면
## 물려받는 남은 시간이 먼저 사슬을 끊는다. 상한 2·3·4의 제대로 된 비교는 tools/spread_probe.gd에 있다.
...
func _init() -> void:
	...
	md += "## 4. 독 전염 — 세대 상한 2에서 사슬이 멈추는가(경로 확인용, 비교표가 아니다)\n\n"
	md += "늑대 16마리를 전염 반경 안에 4×4로 세우고 **가운데 한 마리만** 감염시킨 뒤 그 한 마리를 처치한다.\n"
	md += "**늑대의 체력을 1로 두었다** — 감염된 적이 즉사해야 세대 상한 말고 다른 것이 사슬을 끊지 않기 때문이다.\n"
	md += "비교를 위해 **메모리에 올라온 자격표의 `gen_max`만 잠시** 99로 바꿨다(`data/supports.json` 파일은 읽기만 했다).\n\n"
	md += "> **이 표의 비율을 전투 전체의 효과 감소율로 옮겨 적지 마라.**\n"
	md += "> 체력을 1로 둔 이 조건에서만 세대 상한이 유일한 제동 장치가 된다. 체력이 실제 값에 가까워지면\n"
	md += "> **물려받는 남은 시간**이 먼저 사슬을 끊어 상한을 올려도 아무 변화가 없다.\n"
	md += "> 상한 2·3·4의 제대로 된 비교(밀집한 약한 적 / 튼튼한 적 · 격리 시험실 / 실전 처리 비용)는\n"
	md += "> `tools/spread_probe.gd`(결과 `docs/sim/SPREAD_PROBE.md`)에 있다.\n\n"
