...
- Q1(C4) 다수 편성 변환: 숲 새벽 25마리 기준 배율(×5)을 전 지역에 적용(예: 능선 2일차 10마리 편성 → 50마리) + 역할별 동시 상한 — 이 방식으로 갈지, 지역별 전체 수를 따로 정할지. **사용자 답변(2026-09-07)**: 일괄 ×5는 비교 설정으로 유지(최종 아님). 비교 후보 `roles`(근접 ×5·원거리 ×2·지원/봉쇄 ×1~2·정예/구조물/보스 소환 ×1, 동시 상한 불변, 경험치는 종류별 HTML 예산 ÷ Godot 개체 수)를 데이터 세트로 기록·선택 가능(`FORMATION_TABLE.md`). 첫날 새벽 늑대 25 보존. 잔류 투사체·장판 겹침은 관찰 항목. 두 세트 비교는 빌드 완성 뒤.
