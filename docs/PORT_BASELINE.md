...
| C4 | 편성 밀도: HTML 웨이브 2~4마리 순차 vs Godot 25/12 연속 보충(D33 채택) | DAY_WAVES vs RULES §밀도 | 전투 길이·압박 | **잠정 규칙**: 종류별 전체 수 = HTML 편성 합 × 밀도 배율(숲 1일차가 정확히 25마리가 되는 값 25/9), 동시 상한 12, 묶음 3·간격 1.0, 원거리·지원 역할은 종류별 동시 상한(궁수 3·주술사 1·서리술사 2·거미 2·폭탄 3·방패병 3·멧돼지 2·잠복충 2·도적 3, 시험값). 경험치·금화 예산은 HTML 편성 기준 고정 → **사용자 판단 필요(Q1)** |
...
- Q1(C4) 다수 편성 변환: 숲 25마리 기준 배율(×2.78)을 전 지역에 적용 + 역할별 동시 상한 — 이 방식으로 갈지, 지역별 전체 수를 따로 정할지.
