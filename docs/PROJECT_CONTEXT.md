...
| Godot 구현 범위 | **첫 전투 하나**: 검격 Lv1·이동·회피·Q·늑대(물기·돌진)·바위/나무·HUD·승패·재시작·일시정지·F3 비교 설정. 성장(레벨업)·상점·날짜·보스·E·칼날·창·저장은 **없다**. 경험치는 예산 9.0 누적 계산만 있다 |
| Godot 버전 | godot-0.3.1. 코드·문서 커밋 2cb7ae6, 해시 기록 8cad17f. 배포물 ZIP은 0.3.0(272608f) 그대로 |
...
| 다음 작업 후보 | 사용자 제기: 임시 그래픽, 짧은 전투 시간, 다른 역할의 몬스터 추가. 그 밖에 제목 부제 문구 갱신, OS 간 결정성 확인, 0.3.1 Windows 빌드 ZIP, 초안의 CURRENT_DESIGN_DRAFT를 GAME_SPEC·ASSUMPTIONS에 병합(§6). 우선순위는 `HANDOFF.md` |

전체 성장·경제·지역·3보스 설계는 목표로 보존하되(D03~D26, `GAME_SPEC.md`), Godot 첫 전투의 완료 조건과 구분한다. 한꺼번에 이식하지 않고 `PORT_NOTES.md` §7 순서로 한 덩어리씩.
