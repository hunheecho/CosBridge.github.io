...
| m:boss:gate_warden | 성문 파수장 · 신규 1막 보스(버려진 요새) | 체력 시험값 2400(hi)/1500(base), 단계 70/35%, 방패 자세(0.8/0.3, 자세 중 정면 ±60° 직접 피해 −40%, 실시간 상한 3초, 0 아님) 뒤 밀치기(r110 100° 14 빈틈 1.2)·방패 돌파(0.8/0.4 440·780 18, 2단계~ 끝에 넓은 휩쓸기 0.5/0.3 r150 200° 16 뒤 빈틈 2.4, 1단계 빈틈 2.0)·석궁 3발(0.7/0.35 ±18° 540 10 빈틈 1.4). 투사체 반사 없음 | 감속장 전 단계, 넉백 20% | 계획 §7(패턴·필수 판정), 수치 시험값 | 없음(신규) | 구현(boss3.gd·bosses_new.json·render.gd; boss3_tests) | 포함 | boss3_tests 111 |
| m:boss:spore_matriarch | 포자 어미 · 신규 1막 보스(포자 정원) | 2400/1500, 70/35%, 포자 탄(0.7 조준, 표시 자리 r62에 1.0초 뒤 14, 2단계~ 두 자리 0.45초 차, 잔류 구름 3.5초·동시 3, 빈틈 1.3)·포자 고리(0.9/0.3, 250/s 최대 340 폭 44 16, 빈 구간 70°/70°/55°가 실제 판정에도 비어 있음, 빈틈 1.6)·느린 접근 분사(0.6/0.25 r120 90° 12, 뒤 이동 정지 1.4)·소환 포자 1(동시 2·총 4) | 착탄 시계는 위치 예고(감속장 무관, 먹는 자 표식과 동일) | 계획 §7, 시험값 | 없음(신규) | 구현(boss3.gd·bosses_new.json·render.gd; boss3_tests) | 포함 | boss3_tests 111 |
| m:boss:excavation_behemoth | 굴착 거수 · 신규 2막 보스(무너지는 광산) | 5000/3000, 70/35%, 굴착 돌파(0.9/0.4 480·720 20, 몸 보임·피격 가능, 2단계~ 두 방향 1→2 사전 예고·재조준 없음, 빈틈 2.4/3.0)·낙석 3구역(0.6 뒤 예고 0.9, 0.5초 간격 순차, r72, 이동 방향으로 125 간격, 18, 잔해 4초 5/0.5초, 출구 검사 16방향 ≥4, 빈틈 1.5)·폭탄 운반체 소환 2(동시 2·총 6, 폭발/사망 1회)·호위 동시 위험 행동 1(overlap_limit) | 낙하 시계는 위치 예고 | 계획 §7, 시험값 | 없음(신규) | 구현(boss3.gd·bosses_new.json·render.gd; boss3_tests) | 포함 | boss3_tests 111 |
| m:boss:frost_stalker | 서리 추적자 · 신규 2막 보스(얼어붙은 협곡) | 5000/3000, 70/35%, 얼음 발사(0.6/0.3 720 14 빈틈 1.2)·얼음길 3줄(0.8/0.3 ±40° 420×70 12 + 빙판 r38 7초 감속 0.6, 동시 36, 빈틈 1.5)·옆 이동 140(0.4초, 피격 가능) 뒤 돌진(0.7/0.35 480·850 18 빈틈 2.2), 2단계~ 얼음길→돌진 연계 | 빙판은 걷기 속도만(회피·입력 방향 그대로, 겹침 곱 없음), 아군 냉기 면역 없음 | 계획 §7, 시험값 | 없음(신규) | 구현(boss3.gd·bosses_new.json·render.gd·combat_state.gd ice 1줄; boss3_tests) | 포함 | boss3_tests 111 |
| m:boss:blood_hunt_king | 핏빛 사냥왕 · 신규 3막 보스(피의 사냥터) | 7000/3600, 70/35%, 발톱 휩쓸기(0.7/0.35 r170 170° 20 빈틈 1.5)·추적 돌진 2회(0.7/0.35 420·900 18 → 재조준 0.45(표식) → 재고정 0.3 → 둘째, 빈틈 2.6; 첫 예고 선은 첫 돌진만)·호위 늑대 2(동시 3·총 6, 2단계~ 한쪽 측면 100°) | 늑대는 보스 확정 중 돌진 금지, 보스는 늑대 돌진 중 대기(공유) | 계획 §7, 시험값 | 없음(신규) | 구현(boss3.gd·bosses_new.json·render.gd; boss3_tests) | 포함 | boss3_tests 111 |
| m:boss:doom_executor | 종말의 집행관 · 신규 3막 보스(뒤틀린 성채) | 7000/3600, 70/35%, 세로 절단선 2(1번 0.8초 추적 뒤 고정·0.3 뒤 절단, 2번은 1번 절단 순간 고정·0.8초 뒤, 폭 80 22, 색·번호 구분, 빈틈 1.6)·회전 방어 자세(최대 2.4초, 초당 1.2rad, 정면 ±55° 직접 피해 −40%, 실시간 상한 4초, 느린 접근) 뒤 큰 베기(0.5/0.3 r160 140° 24, 자세 해제 빈틈 2.0)·호위 방패병 2(두 방향, 동시 2·총 4)·호위 동시 위험 행동 1 | 무적·순간이동 없음(장판 유효) | 계획 §7, 시험값 | 없음(신규) | 구현(boss3.gd·bosses_new.json·render.gd; boss3_tests) | 포함 | boss3_tests 111 |
신규 보스 6종 정본: `prophecy_godot/docs/BOSSES.md`(패턴·예고 시간·단계·시험값·필수 판정 상태·봇 검증 범위). 회차 관문/테마 연결(`world.json`·`run.gd`)은 별도 작업.
...
| bot:audit2 | 검수 2 수정: 독립 장판·투사체 새 지연(같은 공격 인스턴스만 상속), 투사체 id 발사 시점 고정·피격 기록 연결, 캐시 키 코드 해시·규칙/관측 버전 | Codex 검수(godot-audit-8dfcbd1) | 없음 | 구현(skill_bot.gd, combat_state.gd stamp_projectile, observe.gd observe-3, hit_recorder.gd, replay.gd code_hash, bot_batch_core.gd) | 포함 | bot_tests §12, BOT_COMPARE_AUDIT2 |
